...
- **MD5 去重上传**：同一文件不会重复上传
- **零运行时依赖配置**：端点固定指向 `https://videoinu.com`，不需要手工切环境
...
```bash
npm install -g videoinu-cli
```

pnpm / yarn 亦可：

```bash
pnpm add -g videoinu-cli
yarn global add videoinu-cli
```

免安装调用：

```bash
npx videoinu-cli <command>
```

安装后执行 `videoinu --help` 验证。
